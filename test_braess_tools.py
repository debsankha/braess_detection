...
class TestAlignmentRing(TestCase):
    ...
    def test_allaligned(self):
        ...
        aligned, nonaligned, cant_say = ab.flow_alignment_with_edge(self.G, self.Gr, (0, 1), flows)
...
class TestAlignmentGrid(TestCase):
    ...
    def test_alignment_all_aligned(self):
        ...
        for u, v in self.Gr.edges_arr:
            if (u,v) in self.true_alignments:
                ...
                flows[(u,v)] = -1
        aligned, nonaligned, cant_say = ab.flow_alignment_with_edge(self.G, self.Gr, self.maxflow_edge, flows)
    ...
    def test_alignment_few_nonaligned(self):
        ...
        aligned, nonaligned, cant_say = ab.flow_alignment_with_edge(self.G, self.Gr, self.maxflow_edge, flows)
        self.assertEqual(len(aligned), self.G.number_of_edges() - len(tobe_notaligned_edges)-1)
...
class TestSteadyFlows(TestCase):
    def test_stflow_compare(self):
        ...
        for _ in range(10):
            ...
            sink_idxs = np.random.choice(list(set(range(I.size)) - set(src_idxs)), num_sinks, replace=False)
...
class TestMaxflowChange(TestCase):
    def test_stuff(self):
        ...
        maxflow_change = ab.maxflow_change_on_each_edge_strengthening(Gr, steady_flows_vec=steadyflows,
                                                                      maxflow_src=maxflow_src, maxflow_sink=maxflow_sink)
        ...
        for u,v in Gr.edges_arr:
            ...
            rel_maxflow_change = (new_stflows[maxflow_edge] - old_stflows[maxflow_edge])/EPS
...
class TestBraessianness:
    def test_be_random(self):
        ...
        sink_idxs = np.random.choice(list(set(range(input_vec.size)) - set(src_idxs)), num_sinks, replace=False)
